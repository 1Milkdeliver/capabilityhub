...
| R19 | Supply-chain trust | Partial | Staged transitions enforce artifact bytes, digest, publisher/registry policy, revocation, expiry and optional local HMAC evidence; public-key/Sigstore identity remains open |
...
| R28 | Multi-client consistency | Partial | Library/HTTP and the exact three MCP tools share one strict service adapter/envelope with correlation conformance; the broad management CLI remains direct and remote HTTP remains out of scope |
