...
- **Decision/answer:** Provide a read-only local dashboard as the primary management surface and a repo-local Codex plugin whose help action returns only a compact status/open-dashboard response. Do not override built-in commands; use `/helpme` for the main menu and `/myskills` for direct Skill access.
