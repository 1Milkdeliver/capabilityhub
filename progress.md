...
- Added a real upstream MCP stdio provider using the official SDK for initialization, tool discovery, invocation, cancellation, and transport; fixed command/tool allowlists, explicit environments, safe stderr handling, deadlines, JSON bounds, and service admission are covered by subprocess tests.
- Upgraded the loopback Dashboard with no-model compact search, project language selection, and enable/disable/quarantine controls backed by the live monitor; added per-process CSRF, same-origin checks, bounded JSON, safe callback failures, and no-secret/no-path response boundaries.
