...
| CLI, MCP and HTTP | Twenty-eight local commands, three MCP tools, and a strict real-service adapter with authenticated loopback HTTP roundtrips | Experimental local interfaces; `http-serve` is a one-process immutable snapshot, connection state is configuration-only, and no remote multi-user API is shipped |
