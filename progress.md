...
- Added atomic SQLite idempotency admission across process restarts, digest-bound conflict detection, crash recovery to uncertain state, default no-output persistence, optional reviewed result replay, and local CLI duplicate blocking without a second provider call.
