...
reference adapters and an in-memory secret-handle broker, but no OS sandbox, authenticated tenant boundary,
...
| Durable budget | SQLite atomic reserve/reconcile/cancel, cross-process hard limits, restart-safe used/reserved state, real project `budget-report`, and a separate HMAC-scoped hierarchical store with atomic ancestor admission | Local runtime uses one scope; hierarchical embedding is not its default and distributed accounting remains open |
...
| Privacy observability | Bounded hashed-correlation spans, low-cardinality metrics, deterministic sampling, and SQLite aggregate retention/export reject raw arguments, output, URLs, paths, secrets, and identities | Embedding core only; not wired into every runtime path and not an external telemetry backend |
