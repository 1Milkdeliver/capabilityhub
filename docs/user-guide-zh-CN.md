...
- 按名称、类型和启用状态筛选完整能力卡片；
- 在每张卡片中查看简短介绍、Provider、开关状态和预计说明 Token；
- 在右上角即时切换中文、英文或跟随系统；
- 从本机 Codex 任务索引选择对话，查看 CapSift 观察到的 Skill 和工具调用；
- 查看 Provider、Routing、Loaded 和连接配置状态；
- 启用、停用或隔离能力；
- 处理已经创建的审批请求；
- 查看脱敏后的审计、上下文和推理状态。

Dashboard 默认只监听 `127.0.0.1`，也就是本机。对话列表只读取轻量的 `session_index.jsonl`；选择对话后只检查工具调用包络，不读取或显示消息、回答和推理正文。超过安全扫描上限的历史会直接标记为不可扫描。

Dashboard 不会自动探测外部服务，也不会显示凭据、完整命令、Skill 正文或 Provider 输出。启用能力本身消耗 `0 Token`；卡片上的 Token 是以后完整加载说明时的估算值。停用能力只能阻止后续通过 CapSift 再次加载，不能删除已经进入 Codex 对话历史的内容。由于 Codex 原生调用不会全部上报给 CapSift，对话视图只显示有证据的调用，不会猜测遗漏项。
