...
| Supply-chain trust | Every forward staged transition re-acquires bytes; policies support local HMAC evidence, pinned Ed25519 keys, or a bounded certificate/transparency bundle with scoped root trust, identity SAN, inclusion proof, signed checkpoint, online freshness, revocation, replay and fork detection | Ed25519/X.509 verification uses the optional `cryptography` extra; the bundle is an explicit CapabilityHub profile, not universal Sigstore-version compatibility or a hosted key-distribution service |
...
| Provider supervision | Local configured execution uses a spawned worker, wall-clock and CPU/memory limits, bounded JSON IPC, Job Object/POSIX process-tree cleanup, and typed failures; production requests for unavailable filesystem/network confinement are rejected before worker creation | No usable filesystem/network confinement backend or long-lived worker pool |
