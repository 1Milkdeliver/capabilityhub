...
| R25 | Hierarchical budgets | Partial | A durable HMAC-scoped SQLite tree enforces child allocation and atomic ancestor admission across restarts; the local runtime still uses its single-scope ledger |
...
| R30 | Observable, private audit | Partial | Redacted views, HMAC audit retention/export, bounded privacy-safe spans and SQLite aggregate metrics exist; default service wiring, external export and OS-backed key management remain open |
