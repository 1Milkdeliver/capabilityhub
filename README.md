...
For stricter production intake, `SigstoreBundleVerifier` accepts a bounded portable certificate/transparency bundle: it validates the pinned certificate root and scope, certificate identity, Ed25519 artifact signature, signed checkpoint, and inclusion proof. Policy can require an online current checkpoint; stale checkpoints, root/log revocation, replay, and same-size log forks fail closed. This is an explicit CapabilityHub bundle profile, not a claim of compatibility with every Sigstore bundle version.
